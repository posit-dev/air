...
Either `true` to format example code, or `false` to leave it untouched, with a default of `false`.
...
The overall `line-width` is respected, meaning that the code is formatted with an adjusted line width equal to `line-width` minus the leading indentation of the roxygen2 block and the leading `#'` comment characters.

Parse failures within an example section are silent and do not affect the parse status of the containing file.

Rd markup, like `\dontrun{}` and `\donttest{}`, is not supported and will result in the entire `@examples` or `@examplesIf` section being left unformatted.
