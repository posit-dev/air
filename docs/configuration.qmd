...
By default, Air will refuse to format files matched by patterns listed in `default-excludes`.
...
-   `folder/` to exclude that directory located anywhere below `root`.
...
-   `/folder/` to exclude a directory at `root/folder/`, where the leading `/` forces the directory to appear right under `root/`, rather than anywhere.

-   `file-*.R` to exclude R files like `file-this.R` and `file-that.R` located anywhere below `root`.

-   `folder/*.R` to exclude all R files in `root/folder/`, where the `/` in the middle of the pattern forces the directory to appear right under `root/`, rather than anywhere.
    Note that R files in directories under `folder/` are not ignored in this case (such as `root/folder/subfolder/file.R`).

-   `folder/**/*.R` to exclude all R files in any directory under `root/folder/`, where the `/` in the middle of the pattern forces the directory to appear right under `root/`, rather than anywhere.

-   `**/folder/*.R` to exclude all R files inside a `folder/` directory, where the `folder/` directory itself can appear anywhere.
