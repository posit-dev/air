...
The set of additional files and folders to ignore.

A list of strings, i.e. `ignore = ["file.R", "folder/", "files-like-*-this.R"]`.

By default, Air will refuse to format files matched by patterns listed in `default-ignore`.
Use this option to supply an additional list of ignore patterns.

Ignore patterns are modeled after what you can provide in a [.gitignore](https://git-scm.com/docs/gitignore), and are resolved relative to the parent directory that your `air.toml` is contained within.
For example, if your `air.toml` was located at `root/air.toml`, then you could provide:

-   `ignore.R` to ignore that R file located anywhere below `root`.
    This is equivalent to `**/ignore.R`.

-   `ignore/` to ignore that directory located anywhere below `root`.
    You can also just use `ignore`, but this would technically also match a file named `ignore`, so the trailing slash is preferred when targeting directories.
    This is equivalent to `**/ignore/`.

-   `/ignore/` to ignore a directory at `root/ignore/`, where the leading `/` forces the directory to appear right under `root/`, rather than anywhere.

-   `ignore-*.R` to ignore R files like `ignore-this.R` and `ignore-that.R` located anywhere below `root`.

-   `ignore/*.R` to ignore all R files in `root/ignore/`, where the `/` in the middle of the pattern forces the directory to appear right under `root/`, rather than anywhere.
    Note that R files in directories under `ignore/` are not ignored in this case (such as `root/ignore/subdir/file.R`).

-   `ignore/**/*.R` to ignore all R files in any directory under `root/ignore/`, where the `/` in the middle of the pattern forces the directory to appear right under `root/`, rather than anywhere.

-   `**/ignore/*.R` to ignore all R files inside an `ignore/` directory, where the `ignore/` directory itself can appear anywhere.

See the full [.gitignore](https://git-scm.com/docs/gitignore) documentation for all of the patterns you can provide.

### default-ignore

Whether or not the default file ignore patterns are used.

Either `true` to use the default ignore patterns, or `false` to not use them, with a default of `true`.

By default, Air ignores a set of files and folders that you are unlikely to want formatting in.
The complete list of default ignore patterns is:
