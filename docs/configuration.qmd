...
-   It stops Air from using user level editor settings.
    When an `air.toml` is not present, Air respects some user level editor settings, such as `editor.tabWidth` in VS Code.
...
If you're happy with Air's defaults and don't plan for your project to ever leave your computer, you likely don't need an `air.toml`.
