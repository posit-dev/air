...
This directive is particularly useful with calls like tibble's `tribble()` or data.table's `fcase()`.
...
By default, all calls to `tribble()` and `fcase()` are formatted as tables, even without a `# fmt: table` directive.
...
-   `default-table`: Whether to turn off Air's default for `tribble()` and `fcase()`.
