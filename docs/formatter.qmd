...
- The layout of the table is never changed by the formatter.
  It never introduces line breaks that don't already exist to avoid messing up the tabular data, which could be hard to recover from.
- Tables are allowed to be jagged with different numbers of columns per rows.
  This allows formatting partial tables as you write them.
